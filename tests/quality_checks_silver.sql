...
-- ====================================================================
-- 生年月日の確認対象を抽出
-- 実行日から120年前より古い日付は要確認とし、不正値とは断定しない。
-- 120年前の同日は対象外。古い日付はSilverで保持し、年齢だけでは補正しない。
-- 未来の日付は不正値（LOAD_SILVERでNULL化されるため、検出されないことを期待）。
SELECT DISTINCT
    bdate
FROM
    silver.erp_cust_az12
WHERE
    bdate < DATEADD(year, -120, CURRENT_DATE())
